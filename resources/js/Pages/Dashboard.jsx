            ...
            <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-4">
                <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div className="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <table
                            className="mt-3 w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            <thead
                                className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 border-b-2 border-gray-500">
                            <tr>
                                <th className="px-3 py-3 text-center">ID</th>
                                <th className="px-3 py-3 text-center">Project Name</th>
                            ...
                            <tbody>
                            {completeTasks.data.map((completed) => (
                                <tr key={completed.id}>
                                    ...
                                    <td className="px-3 py-2 text-white hover:underline">
                                        <Link href={route("project.show", completed.project.id)}>
                                            {completed.project.name}
                                        </Link>
                                    </td>
                                    <td className="px-3 py-2 text-white hover:underline">
                                        <Link href={route("task.show", completed.id)}>
                                            {completed.name}
                                        </Link>
                                    </td>
                                    <td className="px-3 py-2">
                        <span
                            className={
                                ...
                                TASK_STATUS_CLASS_MAP[completed.status]
                            }
                        >
                          {TASK_STATUS_TEXT_MAP[completed.status]}
                        </span>
                                    </td>
                                    <td className="px-3 py-2 text-nowrap">{completed.due_date}</td>
                                </tr>
                            ))}
                            </tbody>
                        </table>
